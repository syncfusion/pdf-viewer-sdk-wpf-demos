    ...
    internal class MicroSoftExtensionAI
    {
        ...
        public async Task<string> SummarizePdf(string extractedText)
        {
            if (AISettings.ClientAI != null)
            {
                ...
                var response = await AISettings.ClientAI.GetResponseAsync(chatMessages);
                return response.ToString();
            }
            return string.Empty;
        }
        ...
        public async Task<string> GetAnswerFromGPT(string systemPrompt, string userText)
        {
            if (AISettings.ClientAI != null)
            {
                ...
                var result = await AISettings.ClientAI.GetResponseAsync(chatMessages);
        ...
        {
            return await GetAnswerFromGPT("You are a helpful assistant. Use the provided PDF document pages and pick a precise page to answer the user question. Provide the answer in plain text without any special formatting or Markdown syntax. Pages: " + PageContents, question);
        }
    }
}
