...
{
    /// <summary>
    /// Represents the main view model for the entire demo browser application. It manages the state, data and interactions for products, samples, themes, navigation and searching.
    /// </summary>
    public abstract class DemoBrowserViewModel : NotificationObject
    {
        ...
        /// </summary>
        public static string HeaderTitleText { get; set; } = "Syncfusion® WPF Demos";

        /// <summary>
        /// Gets or sets a value Whether the current instance is File format libraries.
        /// </summary>
        private static bool isIOSDKSample = false;
        private static bool isDocumentSDKSample = false;

        /// <summary>
        /// To check whether the build is Excel SDK Browser.
        /// </summary>
        public static bool IsIOSDKSample { get => isIOSDKSample; set => isIOSDKSample = value; }

        /// <summary>
        /// To check whether the build is Document SDK Browser.
        /// </summary>
        public static bool IsDocumentSDKSample { get => isDocumentSDKSample; set => isDocumentSDKSample = value; }

        /// <summary>
        /// Gets or sets a value indicating whether this is the Main sample browser application.
        /// </summary>
        public static bool isMainSB = false;

        /// <summary>
        /// Gets or sets the default theme name for the application.
        /// </summary>
        public static string DefaultThemeName = "Windows11Light";

        /// <summary>
        /// Check the UI Culture of the application at startup.
        /// </summary>
        ...
        /// </summary>
        private bool isAISettingsWindowBusy = false;
        ...
        ///// <summary>
        ///// Gets or sets the selected Demo in the AI Sample Demoslist
        ///// </summary>
        //// private DemoInfo _selectedAIDemo;
...
        private Visibility blurVisibility = Visibility.Collapsed;
        ...
        public DemoBrowserViewModel()
        {
            ...
            AISampleDemos = PopulateAISampleDemos();
            if (AISampleDemos.Any())
            {
                AIHilighlightedDemos = PopulateAIHighlightedProductDemos();
            }
            PopulateProducts();
            PopulatePaletteList();
            NavigationItems();
        ...
        {
            get { return isAISettingsWindowBusy; }
            set
            {
                isAISettingsWindowBusy = value;
                RaisePropertyChanged("IsAISettingsWindowBusy");
            }
        }
        ...
        private string navigationButtonContent = "All Controls";

        /// <summary>
        /// Gets or sets the Content for navigation Button.
        /// </summary>
        public string NavigationButtonContent
        {
            get { return navigationButtonContent; }
            set
            {
                navigationButtonContent = value;
                RaisePropertyChanged("NavigationButtonContent");
            }
        }

        private Visibility navigationButtonvisibility = Visibility.Visible;

        /// <summary>
        /// Gets or sets the Visibility state of the navigation button. This property can be used to show or hide the navigation button in the UI.
        /// </summary>
        public Visibility NavigationButtonVisibility
        {
            get { return navigationButtonvisibility; }
            set { navigationButtonvisibility = value; RaisePropertyChanged(nameof(NavigationButtonVisibility)); }
        }
        ...
        /// <summary>
        /// Gets a command to control the Visibility of the theme setting panel
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets the Visibility of the theme button.
        /// </summary>
        ...
        /// </summary>
        public List<ProductDemo> AIHilighlightedDemos { get; set; }
        ...
        /// </summary>
        public List<DemoInfo> AISampleDemos { get; set; }
        ...
        /// </summary>
        private List<ProductDemo> PopulateAIHighlightedProductDemos()
        {
            List<ProductDemo> products = new List<ProductDemo>();
            products.Add(new AIHighlightedProductDemos());
            return products;
        }
        ...
        private List<DemoInfo> PopulateWhatsNewDemos()
        {
            ...
            if(ProductDemos!= null)
            {
                foreach (ProductDemo product in ProductDemos)
                {
                    ...
                    {
                        if (demo.Tag != Tag.None && !demo.IsAISample)
                        {
        ...
        private List<DemoInfo> PopulateAISampleDemos()
        {
            List<DemoInfo> aiDemos = new List<DemoInfo>();
            AISamplesProductDemos  AISamplesProductDemo = new AISamplesProductDemos();
            if (ProductDemos != null)
            {
                foreach (ProductDemo product in ProductDemos)
                {
                    foreach (DemoInfo demo in product.Demos)
                    {
                        if (demo.IsAISample)
                        {
                            DemoInfo aiSampleDemoInfo = new DemoInfo();
                            if (string.IsNullOrEmpty(demo.AISampleDescription))
                            {
                                demo.AISampleDescription = demo.Description;
                            }

                            demo.ProductInfo = AISamplesProductDemo;
                            demo.PreviewTag = product.IsPreview;
                            demo.ControlName = product.Product;
                            AISamplesProductDemo.Demos.Add(demo);
                        }
            ...
            foreach (DemoInfo demoInfo in AISamplesProductDemo.Demos)
            {
                aiDemos.Add(demoInfo);
            }
        ...
        private void OnSelectedProductChanged()
        {
            ...
            if (this.isWindowMode)
            {
                ...
                AISettings.DemoBrowserViewModel = this;
                productDemo = new ProductDemosWindow(this);
                if(!IsIOSDKSample)
                {
                    if (DemosNavigationService.MainWindow == null)
                        DemosNavigationService.MainWindow = productDemo;
                    else
                        productDemo.Owner = DemosNavigationService.MainWindow;
                    SfSkinManager.SetTheme(productDemo, Theme.GetTheme(SelectedThemeName));
                }
            }
            else
            {               
                ...
                //else
                {
                    DemosNavigationService.RootNavigationService.Content = (new DemosListView() { DataContext = this });
                }
            }
            if (SelectedProduct != null && !string.IsNullOrEmpty(SelectedProduct.Product) && SelectedProduct.Product == "Smart AI Solutions")
            {
                NavigationContent = new AISampleOptions();
                if (this.HeaderItems.Count > 0)
                {
                    foreach (NavigationViewModel headerItem in this.HeaderItems)
                    {
                        if (headerItem.NavigationItem == "Smart Components")
                        {
                            this.SelectedItem = headerItem;
                        }
                    }
                }
            ...
            else if (this.SearchItem == null && this.SelectedDemo == null)
            {
            ...
            if (IsIOSDKSample)
            {
                Application.Current.MainWindow = productDemo;
            ...
            else
            {
                ...
                {
                    if (productDemo != null && !productDemo.IsLoaded)
                    {
        ...
        private void OnSelectedSampleChanged(DemoInfo demoInfo)
        {
                   ...
                   (SelectedProduct != null && SelectedProduct.DemoLauchMode == DemoLauchMode.Window))
            {
                ...
                SfSkinManager.SetTheme(demoLauncherView, Theme.GetTheme(SelectedThemeName));
                ...
                DemosNavigationService.DemoNavigationService.Content = (demoLauncherView);
            }
            else
            {
                ...
                {
                    SfSkinManager.SetTheme(DemosNavigationService.DemoNavigationService, SfSkinManager.GetTheme(demoControl));
                    ...
                    DemosNavigationService.DemoNavigationService.Content = (demoControl);
                }
            }
            ...
            if (DemosNavigationService.MainWindow != null && !isIOSDKSample)
            {
                DemosNavigationService.MainWindow.Dispatcher.BeginInvoke(new Action(() =>
                {
                    IsProductDemoBusy = false;
        ...
        {
            if (ThemeButtonVisibility)
            {
                SfSkinManager.SetTheme(DemosNavigationService.DemoNavigationService, Theme.GetTheme(SelectedThemeName));
            }
            var productDemosWindow = Application.Current.Windows.OfType<ProductDemosWindow>();
            foreach (var window in productDemosWindow)
            {
                SfSkinManager.SetTheme(window, Theme.GetTheme(SelectedThemeName));
            }
            ...
            if (navigationService != null && navigationService.Content != null)
            {
                ...
                {
                    SfSkinManager.SetTheme(navigationService.Content as DependencyObject, Theme.GetTheme(SelectedThemeName));
                }
        ...
        /// <summary>
        /// Gets or sets the Observable Collection of available themes for the application. This collection is used to populate the theme selection UI, allowing the user to swicth between different visual style.
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets currently selected theme. When a theme is selected,its name is automatically assigned to <see cref="SelectedThemeName"/>
        /// </summary>
        ...
        private void NavigationItems()
        {
            ...
            NavigationViewModel AISampleOptions = new NavigationViewModel()
            {
                NavigationItem = "Smart Components",
                NavigationIcon = new System.Windows.Shapes.Path()
                {
                    Data = Geometry.Parse("M0.5 3V11M2.5 2.5V11.5C2.5 12.6046 3.39543 13.5 4.5 13.5H13.5C14.6046 13.5 15.5 12.6046 15.5 11.5V2.5C15.5 1.39543 14.6046 0.5 13.5 0.5H4.5C3.39543 0.5 2.5 1.39543 2.5 2.5ZM7.29736 3.0692L7.3164 3.15962C7.538 4.21219 8.34103 5.04589 9.38456 5.30677L9.69613 5.38466C9.79243 5.40874 9.79861 5.54327 9.70491 5.57606L9.13274 5.77633C8.22695 6.09336 7.5456 6.85074 7.32579 7.78489L7.29692 7.90759C7.27341 8.00752 7.13261 8.01138 7.10364 7.9129L7.02357 7.64067C6.77277 6.78793 6.13329 6.10414 5.29924 5.79685L4.69828 5.57544C4.60652 5.54164 4.61283 5.40979 4.70741 5.3849L5.03974 5.29745C6.01929 5.03968 6.78431 4.27467 7.04207 3.29512L7.1028 3.06435C7.12964 2.96237 7.27564 2.966 7.29736 3.0692ZM13.0233 9.27292L13.1759 9.31109C13.2722 9.33516 13.2784 9.46969 13.1847 9.50249L12.8344 9.62509C12.1551 9.86287 11.6441 10.4309 11.4792 11.1315C11.4557 11.2314 11.3149 11.2353 11.2859 11.1368L11.2525 11.0233C11.0644 10.3838 10.5848 9.87095 9.95929 9.64048L9.58305 9.50187C9.49129 9.46806 9.4976 9.33621 9.59217 9.31132L9.76467 9.26593C10.4993 9.0726 11.0731 8.49885 11.2664 7.76418L11.2943 7.65813C11.3189 7.56492 11.4523 7.56824 11.4722 7.66257C11.6384 8.45199 12.2406 9.07727 13.0233 9.27292Z"),
                    Stroke = new SolidColorBrush(Colors.Black),
                    HorizontalAlignment = HorizontalAlignment.Center,
                    VerticalAlignment = VerticalAlignment.Center
                }
            ...
            {
                if(this.WhatsNewDemos != null && this.WhatsNewDemos.Any())
                {
                    this.HeaderItems.Add(whatsNew);
                }

                if (this.AISampleDemos != null && this.AISampleDemos.Any())
                {
                    this.HeaderItems.Add(AISampleOptions);
                }
            ...
            else
            {
                ...
                if (this.ShowcaseDemos != null && this.ShowcaseDemos.Any())
                {
        ...
        private void OnSelectionChanged()
        {
            ...
            else if (pagename == "Smart Components")
            {
                NavigationContent = new AISampleOptions();
            }
            ...
            this.NavigationButtonContent = pagename;
        }
        ...
        private void OnClicked(object paramater)
        {
            ...
            if (paramater.ToString() == "ShowAllShowcase")
            {
                if (this.HeaderItems.Count > 0)
                {
                    foreach (NavigationViewModel headerItem in this.HeaderItems)
                    {
                        if (headerItem.NavigationItem == "Showcase")
                        {
                            this.SelectedItem = headerItem;
                        }
                    }
            ...
            else if (paramater.ToString() == "ExploreAllControls")
            {
                if (this.HeaderItems.Count > 0)
                {
                    foreach (NavigationViewModel headerItem in this.HeaderItems)
                    {
                        if (headerItem.NavigationItem == "All Controls")
                        {
                            this.SelectedItem = headerItem;
                        }
                    }
            ...
            {
#if DocumentSDK
                System.Diagnostics.Process.Start(new ProcessStartInfo("https://help.syncfusion.com/document-processing/introduction") { UseShellExecute = true });
#elif GRIDSDK
                ...
                System.Diagnostics.Process.Start(new ProcessStartInfo("https://help.syncfusion.com/wpf/welcome-to-syncfusion-essential-wpf") { UseShellExecute = true });
#elif DIAGRAMSDK
                System.Diagnostics.Process.Start(new ProcessStartInfo("https://help.syncfusion.com/wpf/diagram/overview") { UseShellExecute = true });   
#else
                if(IsDocumentSDKSample)
                    System.Diagnostics.Process.Start(new ProcessStartInfo("https://help.syncfusion.com/document-processing/introduction") { UseShellExecute = true });
                else
                    System.Diagnostics.Process.Start(new ProcessStartInfo("https://help.syncfusion.com/wpf/welcome-to-syncfusion-essential-wpf") { UseShellExecute = true });
#endif      
            ...
            {
#if DocumentSDK
                System.Diagnostics.Process.Start(new ProcessStartInfo("https://github.com/syncfusion/document-sdk-wpf-demos") { UseShellExecute = true });
#elif GRIDSDK
                System.Diagnostics.Process.Start(new ProcessStartInfo("https://github.com/syncfusion/wpf-demos") { UseShellExecute = true });
#elif SPREADSHEETEDITORSDK
				System.Diagnostics.Process.Start(new ProcessStartInfo("https://github.com/syncfusion/spreadsheet-editor-sdk-wpf-demos") { UseShellExecute = true });
#elif PDFViewerSDK
                System.Diagnostics.Process.Start(new ProcessStartInfo("https://github.com/syncfusion/pdf-viewer-sdk-wpf-demos") { UseShellExecute = true });
#elif DOCXEDITORSDK
                System.Diagnostics.Process.Start(new ProcessStartInfo("https://github.com/syncfusion/docx-editor-sdk-wpf-demos") { UseShellExecute = true });
#elif SCHEDULERSDK
                System.Diagnostics.Process.Start(new ProcessStartInfo("https://github.com/syncfusion/wpf-demos") { UseShellExecute = true });
#elif GANTTSDK
                System.Diagnostics.Process.Start(new ProcessStartInfo("https://github.com/syncfusion/wpf-demos") { UseShellExecute = true });
            ...
            else if(paramater.ToString() == "Submit")
            {
                AISettings.CloseAISettingsWindow();
            }
            else if(paramater.ToString() == "SetupAI")
            {
